    ...
    public NewsSummaryListRecyclerView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyle) {
        ...
        addOnScrollListener(new OnScrollListener() {
            ...
            public void onScrollStateChanged(@NonNull RecyclerView recyclerView, int newState) {
                ...
                //不再加载以及滚动到底部时进行加载操作
                if (((MainActivity) recyclerView.getContext()).getPresenter().isNetworkConnected()) {
                    if (!((NewsSummaryListRvAdapter) getAdapter()).isLoading() && !recyclerView.canScrollVertically(1)) {
                        ...
                        adapter.mainActivity.uiSwitchToLoading(true);
                        //在新线程中进行加载操作
                        new Thread(() -> {
                            MainActivity mainActivity = adapter.mainActivity;
                            //一次加载三天内容
                            mainActivity.getPresenter().loadData();
                            mainActivity.runOnUiThread(() -> mainActivity.insertForSummaries());
                            mainActivity.getPresenter().loadData();
                            mainActivity.runOnUiThread(() -> mainActivity.insertForSummaries());
                            mainActivity.getPresenter().loadData();
                            //加载完成，设置为不在加载
                            mainActivity.runOnUiThread(() -> {
                                mainActivity.insertForSummaries();
                                adapter.mainActivity.uiSwitchToNotLoading();
                            });
                        }).start();
                    }
                } else {
